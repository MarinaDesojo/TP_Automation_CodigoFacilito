    ...
    try:
        ...
        if aircraft_id:
            ...
            for attempt in range(retries):
                ...
                print(f"Warning: Failed to delete aircraft {aircraft_id} after {retries} attempts")
        else:
...
@pytest.fixture
def create_clear_aircraft_fail_not_authenticated(aircraft_data, auth_headers):
    r = api_request(method="POST", path=AIRCRAFTS, json=aircraft_data)
    aircraft_created_status_code = r.status_code

    yield aircraft_created_status_code
    try:
        aircraft_created = r.json()
        aircraft_id = aircraft_created.get('id')
        if aircraft_id:
