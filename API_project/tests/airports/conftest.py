...
@pytest.fixture
def create_clear_airport_fail_not_authenticated(auth_headers, airport_data_1):
    MAX_RETRIES = 5
    iata_code = airport_data_1["iata_code"]
    for attempt in range(MAX_RETRIES):
        get_airport = api_request(method="GET", path=f'{AIRPORTS}/{iata_code}', headers=auth_headers)
    ...
    airport_created = api_request(method="POST", path=AIRPORTS, json=airport_data_1)
    airport_created_status_code = airport_created.status_code

    yield airport_created_status_code
