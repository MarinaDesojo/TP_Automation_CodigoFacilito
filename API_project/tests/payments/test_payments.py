...
from API_project.tests.payments.test_schema import good_payment_data, bad_payment_amount_method_data, payment_schema, payment_schema_array, random_payment_data
...
@pytest.mark.parametrize('bad_payment_data', bad_payment_amount_method_data)
def test_create_payment_fail_negative_flow(create_payment_negative, bad_payment_data, auth_headers):
    payment_creation = api_request(method="POST", path=PAYMENTS, json=create_payment_negative, headers=auth_headers)
    bad_payment_creation_json = payment_creation.json()
    bad_payment_creation_status_code = payment_creation.status_code

    assert bad_payment_creation_status_code in (400, 422), f"Expected 400 or 422, got {bad_payment_creation_status_code}, for payment data {bad_payment_creation_json}"
...
#
# @pytest.mark.parametrize('airport_data_1, airport_data_2, aircraft_data', [(good_airport_data_1, good_airport_data_2, good_aircraft_data)])
# def test_create_payment_fail_negative_flow(create_payment, auth_headers):
#     payment_data, payment_creation_status_code, payment_creation_json = create_payment
