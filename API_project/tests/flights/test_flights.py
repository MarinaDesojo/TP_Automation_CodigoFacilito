...
def test_create_clear_flight(auth_headers):
    airport1 = api_request(method="POST", path=AIRPORTS, json=good_airport_data_1, headers=auth_headers)
    airport1.raise_for_status()
    airport1_json = airport1.json()
    origin = airport1_json.get("iata_code")

    airport2 = api_request(method="POST", path=AIRPORTS, json=good_airport_data_2, headers=auth_headers)
...
def test_create_clear_flight_1(create_clear_airport_1, create_clear_airport_2, create_clear_aircraft, auth_headers):
